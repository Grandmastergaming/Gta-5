 ...
 * Advanced Music Command (aoi.js | ver. 4.2.0 - 4.2.1)
   server.js
   ...
   Last update July 16 2021 - 09.53 WIB (GMT +7)
 */
...
bot.variables({
  ...
  errorjoin: "{title:⚠ You need to join the voice channel first!} {color:FFFF00}",
  errorqueue: "{title:❌ Nothing song was playing.} {color:FF0000}",
  leavevc: "Leaved.",
...
$endif
$let[song;$playSong[$message;$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;0s];1;120s];2;7d];yes;$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;yes];1;yes];2;no];No result.]]
$joinVC[$voiceID]
$setGlobalUserVar[commanduserused;$sum[$getGlobalUserVar[commanduserused];1]]
$onlyIf[$replaceText[$replaceText[$checkCondition[$getServerVar[userid]==default];true;$authorID];false;$getServerVar[userid]]==$authorID;{title:❌ You cant use this command} {color:$getVar[color]}]
...
bot.musicStartCommand({
  channel: "$channelID",
  code: `$if[$getGlobalUserVar[vol;$songInfo[userID]]>=$sum[$getServerVar[maxvol];1]]]
...
bot.musicStartCommand({
  channel: "$channelID",
  code: `$if[$queueLength==1]
$awaitReaction[$songInfo[userID];3m;{title:Reaction Music Control} {footer: ⏹ Stop (3 Minutes)} {color:$getVar[color]};⏹;stop;;yes]
$onlyBotPerms[addreactions;Failed adding reactions. **Missing Permission**]
$onlyIf[$checkContains[$getGlobalUserVar[logmusic;$songInfo[userID]];0;1]!=true;]
$elseIf[$queueLength>=2]
$awaitReaction[$songInfo[userID];3m;{title:Reaction Music Control} {footer: ⏹ Stop | ⏭ Skip | 🔀 Shuffle (3 Minutes)} {color:$getVar[color]};⏹,⏭,🔀;stop,skip,shuffle;;yes]
$onlyBotPerms[addreactions;Failed adding reactions. **Missing Permission**]
$onlyIf[$checkContains[$getGlobalUserVar[logmusic;$songInfo[userID]];0;1]!=true;]
$endelseif
$endif
$if[$getGlobalUserVar[saveseek;$songInfo[userID]]!=0]
...
$setServerVar[linkdownload;$replaceText[$songInfo[url];https://youtube.com/;https://ssyoutube.com/]]
$else
...
bot.musicEndCommand({
  channel: "$channelID",
  code: `$setServerVar[filters;none]
$title[Nothing song again on queue, to play it.]
$footer[Leaved VC.]
$color[$getVar[color]]`
});
...
$thumbnail[$songInfo[thumbnail;1]]
$addField[24/7;\`$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247;$songInfo[userID]];0;off];1;off];2;on]\`;yes]
$addField[Loop;\`$replaceText[$replaceText[$checkContains[$loopStatus;song];true;on - $loopStatus];false;off]\`;yes]
...
$let[song;$playSong[$uri[decode;$splitText[7]];$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;0s];1;120s];2;7d];yes;$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;yes];1;yes];2;no];No result.]]
$textSplit[$message;/]
$else
$let[song;$playSong[$message;$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;0s];1;120s];2;7d];yes;$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;yes];1;yes];2;no];No result.]]
$endif
...
bot.command({
  name: "clean",
  code: `$if[$message[1]==]
$addField[clean --128MB;Running on 200MB+ and cleaned *128MB memory nodes;yes]
$addField[clean --256MB;Running on 400MB+ and cleaned *256MB memory nodes;yes]
$addField[clean --512MB;Running on 700MB+ and cleaned *512MB memory nodes;yes]
$color[$getVar[color]]
$footer[Doing this make WebSocket stop working]
$elseIf[$toLowercase[$message[1]]==--128mb]
$createFile[$repeatMessage[8000000;test];clean.txt]
$wait[500ms]
$addCmdReactions[✅]
$endelseif
$elseIf[$toLowercase[$message[1]]==--256mb]
$createFile[$repeatMessage[16000000;test];clean.txt]
$wait[500ms]
$addCmdReactions[✅]
$endelseif
$elseIf[$toLowercase[$message[1]]==--512mb]
$createFile[$repeatMessage[32000000;test];clean.txt]
$wait[500ms]
$addCmdReactions[✅]
$endelseif
$endif
...
$endif
$joinVC[$replaceText[$replaceText[$checkCondition[$message!=];false;$voiceID];true;$findChannel[$message]]]
$setGlobalUserVar[commanduserused;$sum[$getGlobalUserVar[commanduserused];1]]
...
[Thumbnail]($songInfo[thumbnail])
[Download]($getServerVar[linkdownload]);yes]
$addField[Ping;\`$dbPingms\`;yes]
$addField[24/7;\`$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247;$songInfo[userID]];0;off];1;off];2;on]\`;yes]
$addField[Song;\`$queueLength\`;yes]
$addField[Volume;\`$volume% - $getServerVar[maxvol]%\`;yes]
...
$textSplit[$songInfo[duration;$sub[$queueLength;1]]; ]
$endif
$let[song;$playSong[$getGlobalUserVar[$message[1]];$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;0s];1;120s];2;7d];yes;$replaceText[$replaceText[$replaceText[$getGlobalUserVar[247];0;yes];1;yes];2;no]]]
$joinVC[$voiceID]
$botTyping
$setGlobalUserVar[commanduserused;$sum[$getGlobalUserVar[commanduserused];1]]
$onlyIf[$replaceText[$replaceText[$checkCondition[$getServerVar[userid]==default];true;$authorID];false;$getServerVar[userid]]==$authorID;{title:❌ You cant use this command} {color:$getVar[color]}]
